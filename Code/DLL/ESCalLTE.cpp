#include "StdAfx.h"
#include "ESCalLTE.h"
RF_IMPLEMENT_DYNCREATE(CESCalLTE)

CESCalLTE::CESCalLTE(void)
{
	m_mapClassFunc[_T("ESCalLTEMainFunc")]=&CESCalLTE::ESCalLTEMainFunc;
	m_mapClassFunc[_T("TxLinearizerFunc")]=&CESCalLTE::TxLinearizerFunc;
	m_mapClassFunc[_T("TxRxFreqCompFunc")]=&CESCalLTE::TxRxFreqCompFunc;
	m_mapClassFunc[_T("RxFreqCompFunc")]=&CESCalLTE::RxFreqCompFunc;
	m_mapClassFunc[_T("GenerateAndWriteNV")]=&CESCalLTE::GenerateAndWriteNV;
}
...
bool CESCalLTE::DoProcess()
{
	return CALL_MEMBER_FN(*this, m_fpMainFunc)();
}

bool  CESCalLTE::Handle()
{
	CString strMsg;
	if(!DoProcess())
	{
...
bool CESCalLTE::ESCalLTEMainFunc()
{
	if(!SetLTECalLoss())
		return false;
...
bool CESCalLTE::TxLinearizerFunc()
{
	CString strMsg;
	int iErrorcode;
	...
	int iDLChannel=CLTEBandInfo::GetDLChannel(m_iBand,m_iTxLinChannel);
	dUlMHz=CLTEBandInfo::ArfcnToHz(m_iBand,m_iTxLinChannel);
	...
	for(int iLoopIndex=0;iLoopIndex<myTxLinStruct.iNumPAStates;++iLoopIndex)
	{
		...
		stSweep[iSegmentNum].paState = 255; // no-op
		stSweep[iSegmentNum].paBias = 511;
		stSweep[iSegmentNum].txWaveform=m_iTxWaveform;
		stSweep[iSegmentNum].rgi = 0;
		stSweep[iSegmentNum].numHdetAvg=0;
		stSweep[iSegmentNum].numLpmHdetAvg=0;
	...
	if ((iErrorcode=m_pFunLTECalDev->IWCDMAESCalLinearconfig(0,10,iSegmentTimeMS,dvTRxFreq,dvTxLvlRef,dvRxLvl,iSegmentNum-iNumConfigSigment,1))!=DEVICE_SUCCESS)
	{
	...
	DblVtr dvTRxResult;
	dvTRxResult.clear();
	if ((iErrorcode=m_pFunLTECalDev->IWCDMAFetchTXRXResults(dvTRxResult))!=DEVICE_SUCCESS)
	{
...
bool CESCalLTE::RxFreqCompFunc()
{
	CString strMsg;
	int iErrorcode;
	double  dUlMHz,dDLMhz;	
	short iLNAValue=0;
	int iMode_id=FTM_MODE_ID_LTE;
	int iMode_id1=(0==m_iRXPath)?FTM_MODE_ID_LTE:FTM_MODE_LTE_RX1;
	int iFailflag=0;
	...
	aRX_MeasuredChannels.iRefChannel=CLTEBandInfo::GetDLChannel(m_iBand,m_iTxLinChannel);
	...
	if((iErrorcode=m_pPhone->FTM_SET_Cal_STATE(1))!=PHONE_SUCCESS)
	{ 
		strMsg.Format("FTM_SET_CALIBRATION_STATE Fail!");
		PHONE_CAL_ERR_LOG(strMsg,iErrorcode)
	}
	...
	{		
		...
		int iDLChannel=CLTEBandInfo::GetDLChannel(m_iBand,iChannel);
		aRX_NV_Channels.aiNVChannels[iCHIndex]=iDLChannel;
		aRX_MeasuredChannels.aiMeasuredChannels[iCHIndex]=iDLChannel;
		dUlMHz=CLTEBandInfo::ArfcnToHz(m_iBand,iChannel);
		dDLMhz=CLTEBandInfo::ArfcnToHz(m_iBand,iDLChannel)+0.5;		
		...
		if(DEVICE_SUCCESS!=m_pFunLTECalDev->IWCDMACalSetXOSig(dUlMHz,dDLMhz,0,-74))
		{
		...
		if(DEVICE_SUCCESS!=m_pFunLTECalDev->ICDMACalSet_DLPower_On(true))
		{
		...
		for (int iPWIndex=0;iPWIndex<m_ivRxAction.size();iPWIndex++)
		{
			...
			if(DEVICE_SUCCESS!=m_pFunLTECalDev->ICDMACalSet_DLPowe(dRxPower))
			{
			...
			//**********************************************************
			Sleep(15);			
			short iInputPower=(short)(dRxPower*10);
	...
	if(DEVICE_SUCCESS!=m_pFunLTECalDev->ICDMACalSet_DLPower_On(false))
	{
		LogTrace(MSG_ERROR,"Set DL Power ON fail");
		CalFailLog(_T("_"),_T("_"),CDMACAL_SETDLPOW,"Set DL Power ON fail");
		return false;
...
bool CESCalLTE::TxRxFreqCompFunc()
{
	CString strMsg;
	int iErrorcode;
	inumTRxFreq=0;
	...
	USHORT iSegmentTimeMS=2;
	memset(tx_rx_Sweep,0,256*sizeof(FTM_Cal_Sweep_Request));
	memset(tx_rx_Response,0,256*sizeof(FTM_Cal_Sweep_Response));
	...
	{	
		...
		dvRxLvl.push_back(m_dvRxPower[0]);
	}
	for (int i=NUM_TX_FREQ;i<NUM_SEGMENT_PER_SEQUENCE;i++)
	{
		...
		myTRxRequest[i].expRxAGC= 10*m_dvRxPower[i-NUM_TX_FREQ];
	...
	for (int i=0;i<256;i++)
	{
		...
		tx_rx_Sweep[i].numRBs = 6;  
		tx_rx_Sweep[i].startRB = 22;
	...
	for (int iChannelloop=0;iChannelloop<aTX_MeasuredChannels.iNumMeasuredChannels;iChannelloop++)
	{
		...
		int iDLChannel=CLTEBandInfo::GetDLChannel(m_iBand,aTX_MeasuredChannels.aiMeasuredChannels[iChannelloop]);
		aRX_MeasuredChannels.aiMeasuredChannels[iChannelloop]=iDLChannel;		
		aRX_NV_Channels.aiNVChannels[iChannelloop]=iDLChannel;

		double dUlMHz=CLTEBandInfo::ArfcnToHz(m_iBand,aTX_MeasuredChannels.aiMeasuredChannels[iChannelloop]);
		double dDLMhz=CLTEBandInfo::ArfcnToHz(m_iBand,iDLChannel)+0.5;
		...
		if (0==iChannelloop)
		{
			tx_rx_Sweep[inumTRxFreq].opcode = 0x0004;
			tx_rx_Sweep[inumTRxFreq].ulChan = aTX_MeasuredChannels.aiMeasuredChannels[iChannelloop];
			tx_rx_Sweep[inumTRxFreq].paState = 255; // no-op
			tx_rx_Sweep[inumTRxFreq].paBias = 511;
			tx_rx_Sweep[inumTRxFreq].txWaveform=m_iTxWaveform;
			tx_rx_Sweep[inumTRxFreq].rgi = 0;
			inumTRxFreq++;
		} 
		else
		{
			tx_rx_Sweep[inumTRxFreq].opcode = 0x0002;//切换channel
			tx_rx_Sweep[inumTRxFreq].ulChan = aTX_MeasuredChannels.aiMeasuredChannels[iChannelloop];
			tx_rx_Sweep[inumTRxFreq].paState = 255; // no-op
			tx_rx_Sweep[inumTRxFreq].paBias = 511;
			tx_rx_Sweep[inumTRxFreq].rgi = 0;
	...
	for (int iLoopIndex=1;iLoopIndex<NUM_TX_FREQ;iLoopIndex++)
	{
	...
	if ((iErrorcode=m_pFunLTECalDev->IWCDMAESCalLinearconfig(0,10,iSegmentTimeMS,dvTRxFreq,dvTxLvlRef,dvRxLvl,NUM_NOP_SIGMENTS_PERCH+NUM_SEGMENT_PER_SEQUENCE,m_ivChannellist.size()))!=DEVICE_SUCCESS)
	{
		LogTrace(MSG_ERROR,"Config Enhanced Sweep Cal equiment fail");
		CalFailLog(_T("_"),_T("_"),CDMACAL_SETCDTP,"Config Enhanced Sweep Cal equiment fail");
		return false;
	...
	if((iErrorcode=m_pPhone->FTM_CFG_AND_EXEC_CAL_SWEEP((void*)tx_rx_Sweep,inumTRxFreq,true,true))!=PHONE_SUCCESS)
	{ 
	...
	//***************************************临时措施,未解决的bug
	//aTx_LimVsFreqMeasured.aiTxMeasuredHDET[15][0]=aTx_LimVsFreqMeasured.aiTxMeasuredHDET[14][0];
	//*********************************************
	...
	for(int iLNAIndex=0;iLNAIndex<aRX_Measured.iNumMeasured_LNA_States;++iLNAIndex)
	{
		for(int iNumPMMode=0;iNumPMMode<m_ivRxPM.size();++iNumPMMode)
		{
			for(int iCHIndex=0;iCHIndex<m_ivChannellist.size();++iCHIndex)
			{
				strMsg.Format("%d	%d	%d	%d",m_ivChannellist[iCHIndex],aRX_Measured.aiLNA_States[iLNAIndex],iNumPMMode,aRX_DVGA_LNA_Cal_NV.aLTELNAOffsetvsFreq.iLNAOffsetVsFreq[iTemp++]);
				LogTrace(MSG_MID,strMsg);
			}
		}
...
bool CESCalLTE::ArrangeMyTxLinStruct(TxlinStruct *txlinstruct)
{
...
bool CESCalLTE::GetTxFreqRGI(IntVtr ivPAState,DblVtr dvPower,int* aiRGIList,double* aiBiasList)
{
	CString strMsg;
...
bool CESCalLTE::GetPowerVsBiasXml(CString xmlpath,int rf_mode,int device,int channel,int paState,double* pwrlist,int* biaslist,int* biasNum )
{
	/*CoInitialize(NULL);
...
bool CESCalLTE::GetPowerVsBiasIni(CString IniName,int rf_mode,int device,int channel,int paState,double* pwrlist,dword* biaslist,int* biasNum )
{
...
bool CESCalLTE::GenerateAndWriteNV()
{
	CString strMsg;
	int iErrorcode;
	...
	{
		tx_lin_nv_gen.ref_chan_list[m_vPAState[i]][1][0]=m_iTxLinChannel;
		tx_lin_nv_gen.upper_bound_chan_list[m_vPAState[i]][1][0]=m_iUpperBoundChan;
	...
	if((iErrorcode=m_pPhone->RFCAL_Complete_TxRxCal(m_iNVModeID,aTx_MuliLin_request,aTx_MuliLin_response,&tx_nv_struct,numRecords,&tx_lin_nv_gen,&numLin,nvPath,true))!=PHONE_SUCCESS)
	{ 
		strMsg.Format("RFCAL_Complete_TxRxCal Fail!");
		PHONE_CAL_ERR_LOG(strMsg,iErrorcode)
	}
	if (numLin==0)
	{
		strMsg.Format("RFCAL_Complete_TxRxCal Fail!numLin=0");
		PHONE_CAL_ERR_LOG(strMsg,iErrorcode)
	}
						   ...
						   ) 
{
	...
	m_iRFmode=CLTEBandInfo::GetPhoneMode(m_iBand);
	m_iNVModeID=CLTEBandInfo::GetNVModeID(m_iBand);
	itrFind=pcSettingMap->find(_T("RFMode"));
	if(itrFind==pcSettingMap->end())
	{
		LogTrace(MSG_MID,"Not Find 'RFMode' from ini file! use default");		
	}
	else
	{
		strValue=itrFind->second;
		m_iRFmode=CStr::StrToInt(strValue);
	}
	itrFind=pcSettingMap->find(_T("NVMode"));
	if(itrFind==pcSettingMap->end())
	{
		LogTrace(MSG_MID,"Not Find 'NVMode' from ini file! use default");

	}
	else
	{
		strValue=itrFind->second;
		m_iNVModeID=CStr::StrToInt(strValue);
	}
	if ((PHONE_MODE_MAX==m_iRFmode)||(NV_MODE_MAX==m_iNVModeID))
	{	
		LogTrace(MSG_FATAL,"Not supppoted band!");
		return false;
	}
...
bool CESCalLTE::SetLTECalLoss()
{
	...
	if(DEVICE_SUCCESS!=m_pFunLTECalDev->ILTECalSetLoss(m_iBand,dvCableLoss))
	{

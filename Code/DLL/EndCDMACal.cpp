#include "stdafx.h"
#include "EndCDMACal.h"
RF_IMPLEMENT_DYNCREATE(CEndCDMACal)

CEndCDMACal::CEndCDMACal()
{
	m_mapClassFunc[_T("EndCDMACalMainFunc")]=&CEndCDMACal::CEndCDMACalMainFunc;
	m_mapClassFunc[_T("EndCDMACalFunc")]=&CEndCDMACal::CEndCDMACalFunc;
}
...
bool CEndCDMACal::DoProcess()
{
	 return CALL_MEMBER_FN(*this, m_fpMainFunc)();
}

bool  CEndCDMACal::Handle()
{
	if(!DoProcess())
	{
	...
	CalPassLog("_","_","End CDMA Cal Pass");

	return true;
}

bool CEndCDMACal::CEndCDMACalMainFunc()
{
...
bool CEndCDMACal::CEndCDMACalFunc()
{
...
/*
	...
	if((iErrorcode=m_pPhone->FTM_SET_CALIBRATION_STATE(0))!=PHONE_SUCCESS)
	{
		    LogTrace(MSG_ERROR,_T(" FTM_SET_CALIBRATION_STATE Fail!"))
			m_iErrorcode=iErrorcode;
			CalFailLog(_T("_"),_T("_"),iErrorcode," FTM_SET_CALIBRATION_STATE Fail!");
			return false;
	}

...
BOOL CMyMSXML::CreateChildNode(XMLNode nodeParent,LPCTSTR path, XMLNode & nodeCreated,LPCTSTR nodeText)
{
	// Get Attributes
	...
	nodeParent->appendChild(m_DomDoc->createTextNode("\n\t"));

	nodeParent->appendChild(newElem);
	if (attrName.size() == 0)
		return TRUE;
...
BOOL CMyMSXML::CreateNode(LPCTSTR path,XMLNode & nodeCreated, LPCTSTR nodeText)
{
	CString dupStr = path;
	XMLNode nodeTemp;
	while (1) {
	...
	node->appendChild(m_DomDoc->createTextNode("\n\t"));
...
BOOL CMyMSXML::DeleteNode(XMLNode node)
{

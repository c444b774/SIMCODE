#include "stdafx.h"
#include "RTR6285WDMACal.h"
RF_IMPLEMENT_DYNCREATE(CQTR6285TXRXCal)

CQTR6285TXRXCal::CQTR6285TXRXCal()
{
	m_mapClassFunc[_T("QTR6285TXRXCalMainFunc")]=&CQTR6285TXRXCal::CQTR6285TXRXCalMainFunc;
	m_mapClassFunc[_T("QTR6285TXLIN_CalFunc")]=&CQTR6285TXRXCal::CQTR6285TX_LIN_CalFunc;
    m_mapClassFunc[_T("QTR6285TXLIMCompen_CalFunc")]=&CQTR6285TXRXCal::CQTR6285TX_LIMComPen_CalFunc;
	m_mapClassFunc[_T("QTR6285RXCalFunc")]=&CQTR6285TXRXCal::CQTR6285RXCalFunc;
	m_mapClassFunc[_T("TXRXComposite")]=&CQTR6285TXRXCal::CQTR6285TXRXComposite;
}
...
bool CQTR6285TXRXCal::DoProcess()
{
	 return CALL_MEMBER_FN(*this, m_fpMainFunc)();
}

bool  CQTR6285TXRXCal::Handle()
{
	if(!DoProcess())
	{
...
bool CQTR6285TXRXCal::CalculateLimPDM(int iExpectPDMNum,double dExpectPower,int iPDMSize,IntVtr & vtrLimPDMList)
{
   CString strMsg;
...
bool CQTR6285TXRXCal::GetTXLinPowerList(int iPRangeIndex,int iStartPDM,double dExpectedPower,int iPDMNum,int iTimeOut,double dStepPower,double dULMHz,DblVtr & dPowerListResults)
{
	  CString strMsg;
	  int iErrorcode;
	  	...
	  	if(DEVICE_SUCCESS!=m_pFunWCDMACalDev->IWCDMACalSetMaxPower(dExpectedPower))
	   {
		...
		if((iErrorcode=m_pPhone->FTM_CONFIGURE_TX_SWEEP_CAL(655,5))!=PHONE_SUCCESS)//para to QSPR
		{ 
...
bool CQTR6285TXRXCal::GetChannelPower_PDM(int iPARange,int iPDM,double dExpectedPwr,double & dMeasuredPwr)
{
	  CString strMsg;
	  int iErrorcode;
...
bool CQTR6285TXRXCal::SetWCDMACALLOSS()
{
   CString strMsg;
     double dLoss;
	m_vCableLoss.clear();
...
bool CQTR6285TXRXCal::CQTR6285RXCalFunc()
{
   ...
   short uLNAOffset;
	...
	aRX_MeasuredChannels.iRefChannel=CWCDMABandInfo::GetDLChannel(m_iCurrBand,m_iRefCH);;
	  ...
	  for(int iCHIndex=0;iCHIndex<m_vRXCalCHList.size();++iCHIndex)
	{
			...
			LogTrace(MSG_MID,strMsg);
			dUlMHz=CWCDMABandInfo::ArfcnToHz(m_iCurrBand,CWCDMABandInfo::GetULChannel(m_iCurrBand,m_vRXCalCHList[iCHIndex]),1);
			 dDLMhz=CWCDMABandInfo::ArfcnToHz(m_iCurrBand,m_vRXCalCHList[iCHIndex],0);
		...
		if((iErrorcode=m_pPhone->FTM_SET_CHAN(CWCDMABandInfo::GetULChannel(m_iCurrBand,m_vRXCalCHList[iCHIndex])))!=PHONE_SUCCESS)
				{ 
	...
	/*	if(iCHIndex==0)
		...
		short sDVGAAGC=-512 + 12*(m_vRXOffsetPower[0]+m_dDyRange);
		...
		{ 
			...
			PHONE_CAL_ERR_LOG(strMsg,iErrorcode)
			  ...
			  if(uDVGAOffset>m_vRXOffsetUpLimit[0] || uDVGAOffset<m_vRXOffsetLowLimit[0])
			 {
			  ...
			  aRX_Measured.aiDVGA_Measured[iCHIndex][0]=uDVGAOffset;
		...
		for(int iRXIndex=1;iRXIndex<m_vRXOffsetPower.size();++iRXIndex)
		{
			...
			short sDVGAAGC=-512 + 12*(m_vRXOffsetPower[iRXIndex]+m_dDyRange);
	...
	if((iErrorcode=m_pPhone->RFCAL_CDMA_RxCal_Results(CWCDMABandInfo::GetPhoneMode(m_iCurrBand),0,1,&aRX_Measured,&aRX_MeasuredChannels,&aRX_NV_Channels,&aRX_DVGA_LNA_Cal_NV,1,1))!=PHONE_SUCCESS)
		{ 
...
bool CQTR6285TXRXCal::CQTR6285TXRXCalMainFunc()
{
...
bool CQTR6285TXRXCal::CQTR6285TXRXComposite()
{
	CString strMsg;
	int iErrorcode;
	...
	int iNumStepsEachChannel=m_iTXLimPDMNum+m_vTXCompPDM.size()+m_vTXSecCompPDM.size();
	aRX_MeasuredChannels.iNumMeasuredChannels=m_vRXCalCHList.size();
	aRX_MeasuredChannels.iRefChannel=CWCDMABandInfo::GetDLChannel(m_iCurrBand,m_iRefCH);
	aRX_Measured.iPath=0;
	aRX_Measured.iNumMeasuredPowerModes=1;
	...
	for(int i=0;i<m_vRXCalCHList.size();++i)
	{
		aRX_MeasuredChannels.aiMeasuredChannels[i]=m_vRXCalCHList[i];
		aRX_NV_Channels.aiNVChannels[i]=m_vRXCalCHList[i];
		dDownlinkCalFreq[i]=CWCDMABandInfo::ArfcnToHz(m_iCurrBand,m_vRXCalCHList[i],0);
	}

	 CalculateSecFreqCompPwr(m_TxLinResForSecFreqComp,m_vTXSecCompPDM,vtrPwrList);
	 for(int i=0;i<vtrPwrList.size();++i)
	 {
		 dMaxPowerList[i+1]=vtrPwrList[i]+10;
	 }
	...
	{ 
	 ...
	 aTx_Measured_Power.iTxCompType=1;
	 ...
	 aTx_Measured_Power.a2ndTxCompPDM[0]=m_vTXSecCompPDM[0];
	...
	for(int j=0;j<iNumStepsEachChannel;j++)
		{
				...
				if(j<(m_vPARange.size()+m_vTXSecCompPDM.size()-1)) 
				{
					  if(j<m_vTXSecCompPDM.size()+1)
					  {
	                      tx_rx_req_res.aiPA_RangeList[j]=m_vPARange[0];
					  }
					  else
					  {
						  tx_rx_req_res.aiPA_RangeList[j]=m_vPARange[j-m_vTXSecCompPDM.size()];
			...
			if(j<m_vTXCompPDM.size()+m_vTXSecCompPDM.size())
			{
				...
				else if(j>0 && j<m_vTXSecCompPDM.size()+1)
                tx_rx_req_res.aiTxPDM_List[j]=m_vTXSecCompPDM[j-1];
				else
				{
			    tx_rx_req_res.aiTxPDM_List[j]=m_vTXCompPDM[j-2];
				dMaxPowerList[j]=m_vTXCompPower[j-2];
				}
			...
			else
			{
	         ...
	         tx_rx_req_res.aiTxPDM_List[j]=iTXLimPDMList[j-m_vTXCompPDM.size()-m_vTXSecCompPDM.size()];	
			  dMaxPowerList[j]=29;
			}
			...
			if(j<m_vRXOffsetPower.size())
			{
				...
				tx_rx_req_res.aiExpectedRxAGC_ValList[j]=-512 + 12*(m_vRXOffsetPower[j]+m_dDyRange);
				if(j==0)
	...
	{ 
		...
		{ 
			 	...
			 	if(DEVICE_SUCCESS!=m_pFunWCDMACalDev->IWCDMAFetchTXRXResults(dTxFreqSweepMeaPWR))
		   {
		    ...
		    int iNumPACompen=m_vTXCompPower.size()+m_vTXSecCompPDM.size();
		   	for(int j=0;j<FTM_TX_RX_FREQ_MAX_FREQUENCIES;j++)
				{
					 ...
					 aRX_Measured.aiDVGA_Measured[j][0]=tx_rx_req_res.aiRxResults[j][0];
							...
							for(int k=0;k<iNumStepEachChannelCalc;k++)
							{
                                ...
                                aTx_Measured_Power.aiTx_sec_comp_meas[0][j]=dTxFreqSweepMeaPWR[j*(iNumStepEachChannelCalc)+k];
								else if(k==2)
                                aTx_Measured_Power.aiTx_sec_comp_meas_next[0][j]=dTxFreqSweepMeaPWR[j*(iNumStepEachChannelCalc)+k];
								else if(k<iNumPACompen)
                                aTx_Measured_Power.aiTxMeasuredPower[k-2][j]=dTxFreqSweepMeaPWR[j*(iNumStepEachChannelCalc)+k];
							...
							if(m_dTXLimPower<aTx_LimVsFreqMeasured.aiTxMeasuredPower[j][iNumPACompen] || m_dTXLimPower>aTx_LimVsFreqMeasured.aiTxMeasuredPower[j][(iNumStepEachChannelCalc-1)])
							{
								strMsg.Format("Channel=%d  Lowest Power=%.2f ,Highest Power=%.2f not inlcude Expected LimPower=%.2f",m_vTXCalCHList[j],aTx_LimVsFreqMeasured.aiTxMeasuredPower[j][iNumPACompen] ,aTx_LimVsFreqMeasured.aiTxMeasuredPower[j][iNumStepEachChannelCalc-1],m_dTXLimPower);
								LogTrace(MSG_ERROR,strMsg);
	...
	if((iErrorcode=m_pPhone->RFCAL_CDMA_Tx_LinVsFreq_Results(CWCDMABandInfo::GetPhoneMode(m_iCurrBand),&aTx_Measured_Power,&aTx_MeasuredChannels,&aTx_NV_Channels,12,0,&aTx_LinVsFreq_NV_Result,1,1))!=PHONE_SUCCESS)
	{ 
	...
	for(int i=0;i<m_vPARange.size();++i)
	{
		...
		for(int j=0;j<m_vTXCalCHList.size();++j)
		{
			strMsg.Format("PA Range=0 Channel=%d 2nd CompFreqValue=%d",m_vTXCalCHList[j],aTx_LinVsFreq_NV_Result.aTx_2ndFreqComp[0].aiTxCompValue[j]);
			LogTrace(MSG_MID,strMsg)  
		}
...
bool CQTR6285TXRXCal::CQTR6285TX_LIMComPen_CalFunc()
{
	...
	{ 
     ...
     CalculateLimPDM(m_iTXLimPDMNum,m_dTXLimPower,m_iTXLimPDMSize,iTXLimPDMList);
	 aTx_Measured_Power.iNumMeasuredTxGainStates=m_vPARange.size();
	 aTx_Measured_Power.iTxCompType=m_iCalSecFreqComp;
	 aTx_Measured_Power.a2ndTxCompHasMeas[0]=1;
	 aTx_Measured_Power.a2ndTxCompHasMeas[1]=0;
	 aTx_Measured_Power.a2ndTxCompHasMeas[2]=0;
	 aTx_Measured_Power.a2ndTxCompHasMeas[3]=0;
	 ...
	 aTx_Measured_Power.a2ndTxCompPDM[2]=0;
	 aTx_Measured_Power.a2ndTxCompPDM[3]=0;
	 if(aTx_Measured_Power.iTxCompType==1)
	 {
		 CalculateSecFreqCompPwr(m_TxLinResForSecFreqComp,m_vTXSecCompPDM,vtrPwrList);
	 }
		   ...
		   dDLMhz=CWCDMABandInfo::ArfcnToHz(m_iCurrBand,CWCDMABandInfo::GetDLChannel(m_iCurrBand,m_vTXCalCHList[iChIndex]),0);
				...
				if(DEVICE_SUCCESS!=m_pFunWCDMACalDev->ICDMACalSet_DLPower_On(false))
			   {
				...
				for(int iPRIndex=0;iPRIndex<m_vPARange.size();++iPRIndex)
				{
					...
					if((iPRIndex==0) && (aTx_Measured_Power.iTxCompType==1))
					{												
						for (int iSecFreqCompIndex=0;iSecFreqCompIndex<m_vTXSecCompPDM.size();iSecFreqCompIndex++)
						{
							if(!GetChannelPower_PDM(m_vPARange[0],m_vTXSecCompPDM[iSecFreqCompIndex],vtrPwrList[iSecFreqCompIndex],dChannelPower))
								return false;
							
							if (iSecFreqCompIndex==0)
							{
								aTx_Measured_Power.aiTx_sec_comp_meas[iPRIndex][iChIndex]=dChannelPower;
								strMsg.Format("TX linSecCompen 1st:Band=%s,Channel=%d,PARange=%d,PDM=%d,Power=%.2f",m_strCurrBand,m_vTXCalCHList[iChIndex],m_vPARange[iPRIndex],m_vTXSecCompPDM[iSecFreqCompIndex],dChannelPower);
								LogTrace(MSG_MID,strMsg) 
							}
							else
							{
								aTx_Measured_Power.aiTx_sec_comp_meas_next[iPRIndex][iChIndex]=dChannelPower;
								strMsg.Format("TX linSecCompen 2nd:Band=%s,Channel=%d,PARange=%d,PDM=%d,Power=%.2f",m_strCurrBand,m_vTXCalCHList[iChIndex],m_vPARange[iPRIndex],m_vTXSecCompPDM[iSecFreqCompIndex],dChannelPower);
								LogTrace(MSG_MID,strMsg) 
							}							
						}
					...
					//TX lim Compen
					aTx_LimVsFreqMeasured.iNumOfTxLimMeasurement[iChIndex]=m_iTXLimPDMNum;
					bool bLowlimit=true,bHilimit=true;
                    for(int j=0;j<m_iTXLimPDMNum;j++)
					{
                   ...
                   LogTrace(MSG_MID,strMsg) 
					...
					if (bHilimit||bLowlimit)
					{
						strMsg.Format("Channel=%d  Powers not overlapn Expected LimPower=%.2f",m_vTXCalCHList[iChIndex],m_dTXLimPower);
						LogTrace(MSG_ERROR,strMsg);
						CalFailLog(_T("_"),_T("_"),WCDMACAL_SETSIG,strMsg);
						return false;
					}
...
bool CQTR6285TXRXCal::CQTR6285TX_LIN_CalFunc()
{
	...
	if (m_iCalSecFreqComp)
	{
		memset(&m_TxLinResForSecFreqComp,0,sizeof(m_TxLinResForSecFreqComp));
	}
	 ...
	 aTx_Measured.iNV_Resolution=12;
	 ...
	 aTx_Measured.iUMTS_TxAGC_PDM_Max=255;
	 aTx_Measured.iUse_HDET_V2=0;
	 aTx_Measured.i_TxAGC_PDM_MinGain=0;
	 aTx_Measured.HDET_OffsetPower_dBm=m_dTXLimPower-8;
	 aTx_Measured.HDET_SpanPower_dBm=m_dTXLimPower+2;	
	...
	aTx_Measured.iTotalNVItems=64;
	 ...
	 LogTrace(MSG_MID,strMsg)
	 strMsg.Format("HDET_MaxLim=255,NV_Resolution=12,Skip_NonMonotonic_Items=0,TxAGC_PDM_Max=255,HDET_OffsetPower_dBm=16,HDET_SpanPower_dBm=26");
	 LogTrace(MSG_MID,strMsg)
	...
	{ 
		...
		for(int iLoopIndex=0;iLoopIndex<m_vPARange.size();++iLoopIndex)
	  {
       ...
       for(int i=0;i<iPDMNum;++i)
	   {
		 ...
		 if (m_iCalSecFreqComp)
		 {		 
			 m_TxLinResForSecFreqComp.aiTxLinPower[iLoopIndex][i]=dMeasuredPowerList[i];
			 m_TxLinResForSecFreqComp.aiTxLinPDM[iLoopIndex][i]=m_vStartPDM[iLoopIndex]-m_vStepSize[iLoopIndex]*i;
			 m_TxLinResForSecFreqComp.iNumTxPowerMeasurementsPerGainState[iLoopIndex]=aTx_Measured.iNumTxPowerMeasurementsPerGainState[iLoopIndex];
		 }
...
bool CQTR6285TXRXCal::CalculateSecFreqCompPwr(QMSL_CDMA_TxLin_BetaScaling_struct TxLinResForSecFreqComp,IntVtr iEumPDM,DblVtr & vtrPwrList)
{
	int iSecFreqCompLoop=iEumPDM.size();
	for (int i=0;i<iSecFreqCompLoop;i++)
	{
		double dTemp;
		for (int j=0;j<TxLinResForSecFreqComp.iNumTxPowerMeasurementsPerGainState[0];j++)
		{
			if (TxLinResForSecFreqComp.aiTxLinPDM[0][j]>=iEumPDM[i] && TxLinResForSecFreqComp.aiTxLinPDM[0][j+1]<=iEumPDM[i])
			{
				dTemp=(TxLinResForSecFreqComp.aiTxLinPower[0][j]-TxLinResForSecFreqComp.aiTxLinPower[0][j+1])/(TxLinResForSecFreqComp.aiTxLinPDM[0][j]-TxLinResForSecFreqComp.aiTxLinPDM[0][j+1])*(iEumPDM[i]-TxLinResForSecFreqComp.aiTxLinPDM[0][j])+TxLinResForSecFreqComp.aiTxLinPower[0][j];
				vtrPwrList.push_back(dTemp);
			}
		}
	}
		...
		) 
 {
  CStringStringMapItr itrFind;
	CString strValue;
	 ...
	 itrFind=pcSettingMap->find(_T("CalSecFreqComp"));
	 if(itrFind==pcSettingMap->end())
	 {
		 LogTrace(MSG_WARNING,"Not Find 'CalSecFreqComp' from ini file!");
		 m_iCalSecFreqComp=1;
	 }
	 else
	 {
		 strValue=itrFind->second;
		 m_iCalSecFreqComp=CStr::StrToInt(strValue);
	 }

	 itrFind=pcSettingMap->find(_T("TXSecComPdm"));
	 if(itrFind==pcSettingMap->end())
	 {
		 LogTrace(MSG_FATAL,"Not Find 'TXSecComPdm' from ini file!");
		 return false;
	 }
	 else
	 {
		 strValue=itrFind->second;
		 CStr::ParseString(strValue,",",strVtr);
		 CStr::StrVtr2IntVtr(m_vTXSecCompPDM,strVtr);
	 }
